...
-- Extra Tables for real-time dashboards
DROP TABLE TRAFFIC;
DROP STREAM customer_clickstream_time;
DROP STREAM clickstream_time;
CREATE TABLE traffic AS SELECT userid, count(*) AS KSQL_COL_0 FROM clickstream WINDOW TUMBLING (SIZE 5 SECONDS) GROUP BY userid;
CREATE STREAM customer_clickstream_time WITH (PARTITIONS=2) AS SELECT userid, u.username, u.first_name, u.last_name, u.city, u.level, u.registered_at, time, TIMESTAMPTOSTRING(c.ROWTIME, 'yyyy-MM-dd HH:mm:ss.SSS') AS rowtimefull, SUBSTRING(TIMESTAMPTOSTRING(c.ROWTIME, 'yyyy-MM-dd HH:mm:ss.SSS'),0,11) as rowtimedate, SUBSTRING(TIMESTAMPTOSTRING(c.ROWTIME, 'yyyy-MM-dd HH:mm:ss.SSS'),11,16) as rowtimeHHMM, ip, request, status, agent, bytes FROM clickstream c LEFT JOIN web_users u ON c.userid = u.user_id;
CREATE STREAM clickstream_time AS SELECT time, ip, request, status, userid, bytes, agent, TIMESTAMPTOSTRING(c.ROWTIME, 'yyyy-MM-dd HH:mm:ss.SSS') AS rowtimefull, SUBSTRING(TIMESTAMPTOSTRING(c.ROWTIME, 'yyyy-MM-dd HH:mm:ss.SSS'),0,11) as rowtimedate, SUBSTRING(TIMESTAMPTOSTRING(c.ROWTIME, 'yyyy-MM-dd HH:mm:ss.SSS'),11,16) as rowtimeHHMM FROM clickstream c ;
